	...
	class likeController extends Controller
	{
		...
		public function 				add ()
		{
			if ( !$this->user_middleware->isSignin( $_SESSION ) ) {
				...
				try {
					if ( $this->gallery_middleware->isImageExist( $_POST["id"] ) ) {
						if ( !$this->like_model->isLikeExists( $_POST["id"], $_SESSION['userid'] ) ) {
							...
							if ( !$this->like_model->unlikeImage( $_POST["id"], $_SESSION['userid'] ) ) {
								$this->viewData = [
									"success" => "false",
									"msg" => "Failed to unlike the image !"
								];
							} else {
								$this->viewData = [ "success" => "true" ];
							}
					...
					$this->viewData = [
						"success" => "false",
						"msg" => "Something goes wrong while like the image !"
					];
				}
			}
		...
		public function					userswholikes ()
		{
			try {
				...
				$this->viewData = [
					"success" => "false",
					"msg" => "Something goes wrong while get users who liked the image !"
				];
			}
