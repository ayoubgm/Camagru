			...
			try {
				if ( !$this->user_middleware->isSignin( $_SESSION ) ) {
					...
					if ( !$this->gallery_middleware->isImageExist( $_POST["id"] ) ) {
						...
						$this->viewData = [
							"success" => "false",
							"msg" => "Failed to save the comment !"
						];
					}
				}
			} catch ( Exception $e ) {
				$this->viewData = [
					"success" => "false",
					"msg" => "Something is wrong while save the comment !"
				];
			}
				...
				if ( ( isset( $_POST["token"] ) && !empty( $_POST["token"] ) ) && $this->user_middleware->validateUserToken( $_POST["token"] ) ) {
					try {
						if ( $error = $this->comment_middleware->delete( $_POST["id"] ) ) {
							...
							$this->viewData = [
								"success" => "true",
								"msg" => "The comment deleted successfully !"
							];
						} else {
							$this->viewData = [
								"success" => "false",
								"msg" => "Failed to delete the comment successfully !"
							];
						}
					} catch ( Exception $e ) {
						$this->viewData = [
							"success" => "false",
							"msg" => "Something is wrong while delete the comment !"
						];
					}
				}
			}
